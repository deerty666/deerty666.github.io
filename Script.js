...
/* ====== 🔄 دالة المنيو بنظام الأكورديون (الأقسام المغلقة افتراضياً) ====== */
function renderAccordionMenu(searchTerm = '') {
    menuList.innerHTML = ''; 
    const normalizedSearch = searchTerm.trim().toLowerCase();

    processedMenuData.forEach((sec) => {
        // 🚫 تخطي القسم إذا لم يكن متوفراً في الفرع الحالي
        if (sec.section !== "الكل" && sec.sectionAvailableIn && !sec.sectionAvailableIn.includes(currentBranchId)) {
            return; 
        }

        // 🔍 تصفية الوجبات بناءً على البحث والفرع الحالي
        let itemsToRender = sec.items.filter(item => {
            const matchesBranch = item.availableIn && Array.isArray(item.availableIn) && item.availableIn.includes(currentBranchId);
            const matchesSearch = item.name.toLowerCase().includes(normalizedSearch);
            return matchesBranch && matchesSearch;
        });

        if (normalizedSearch.length > 0 && itemsToRender.length === 0) {
            return;
        }

        const sectionDisplayName = sec.section === "الكل" ? `فرع ${currentBranch.name}` : sec.section;

        // 📦 إنشاء حاوية القسم الخارجي (مغلقة افتراضياً)
        const sectionWrapper = document.createElement('div');
        sectionWrapper.className = 'accordion-section';

        // 🏷️ إنشاء رأس القسم (شريط الضغط)
        const sectionHeader = document.createElement('div');
        sectionHeader.className = 'accordion-header';
        sectionHeader.innerHTML = `
            <div class="accordion-title-side">
                <img src="${sec.sectionImg}" alt="${sec.section}" onerror="this.style.opacity=.35" class="accordion-sec-img">
                <span class="accordion-sec-name">${sectionDisplayName}</span>
            </div>
        ...
        itemsContainer.className = 'accordion-content';

        if (itemsToRender.length === 0 && sec.section !== "الكل") {
            itemsContainer.innerHTML = `<p class="no-items-text">لا تتوفر وجبات حالياً</p>`;
        } else {
            itemsToRender.forEach(item => {
                ...
                let priceDisplay = hasDiscount ? 
                    ...
                    (item.basePrice > 0 ? `${item.basePrice} ريال` : `${item.options[0]?.price || 0} ريال`);

                const displayedSection = item.actualSection || sec.section;

                const itemCard = document.createElement('div');
                itemCard.className = 'card' + cardClassAddition;
                itemCard.innerHTML = `
                    <img src="${item.img}" alt="${item.name}" onerror="this.style.opacity=.35">
                    ${bestSellerBadge}
                    <h3 style="margin-bottom: 2px;">${item.name}</h3>
                    <h4 style="color: #888; font-weight: normal; font-size: 0.85rem; margin-top: 0; font-family: sans-serif;">${item.nameEn || ''}</h4>
                ...
                if (isAvailable) {
                    itemCard.querySelector('button').onclick = function(e) {
                        e.stopPropagation(); // 🛑 يمنع إغلاق القسم عند الضغط على زر السلة
                        ...
                        itemNoteInput.value = '';
                        showOptions(itemForCart, !needsOptions, itemImage);
                    };
                }
        ...
        // ⚡ فتح وإغلاق القسم عند النقر
        sectionHeader.onclick = () => {
            const isOpen = sectionWrapper.classList.contains('open');
            
            // إغلاق أي قسم آخر مفتوح لتركيز العميل على قسم واحد
        ...
        menuList.appendChild(sectionWrapper);
    });
}
...
sendWhatsapp.addEventListener('click', () => {
    ...
    cart.forEach(it=>{
        ...
        lines.push(`${count}️⃣ ${it.name} × ${it.qty}`);
        if(optionText){
...
/* 🛠️ ربط شريط البحث بالدالة البديلة الجديدة للأكورديون */
searchBar.addEventListener('input', (e) => {
    renderAccordionMenu(e.target.value);
});

// ====== منطق الـ Sticky Header (التثبيت مع الظل) ======
...
function updateCurrentBranchUI(){
    currentBranchName.innerText = currentBranch.name;
}

document.querySelectorAll('.branch-option').forEach(option => {
    option.addEventListener('click', () => {
        currentBranchId = option.dataset.branch;
        localStorage.setItem('selected_branch', currentBranchId);

        const newBranch = BRANCH_CONFIG[currentBranchId];
        currentBranch.whatsapp = newBranch.whatsapp;
        currentBranch.name = newBranch.name;
        currentBranch.deliveryFee = newBranch.deliveryFee;

        updateCurrentBranchUI();
        renderAccordionMenu(); // 🔄 تشغيل دالة الأكورديون البديلة هنا عند تغيير الفرع
        renderCart();
...
updateCurrentBranchUI();
renderAccordionMenu(); // 🚀 تشغيل دالة الأكورديون البديلة عند أول تحميل للموقع
renderCart();

// تسجيل العامل الخدمي (Service Worker)
...
// ====== دالة تأثير سقوط الصورة إلى السلة ======
